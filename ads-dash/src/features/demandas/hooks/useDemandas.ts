...
export interface UseDemandasReturn {
  ...
  moverPara: (id: string, status: DemandaStatus, ordem?: number) => Promise<void>;
...
export function useDemandas(): UseDemandasReturn {
  ...
  const moverPara = useCallback(async (id: string, status: DemandaStatus, ordem?: number) => {
    await atualizar({ id, status, ordem: ordem ?? Date.now() });
  }, [atualizar]);
  ...
  const porStatus = useMemo(() => {
    ...
    // Backlog + fazendo: ordem manual (arrastar reordena).
    // Feito: concluido_em desc — recém-concluído no topo.
    const porOrdem = (a: Demanda, b: Demanda) => a.ordem - b.ordem;
    ...
    buckets.backlog.sort(porOrdem);
    buckets.fazendo.sort(porOrdem);
