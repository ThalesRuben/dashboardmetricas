-- 0018_whatsapp_summary_period.sql
-- Estende get_whatsapp_summary com janela de período (p_from, p_to).
-- Todos os KPIs ficam filtrados pelo período EXCETO `agendamentos` — que
-- continua sendo o total existente hoje (decisão de produto: user quer
-- sempre ver a fila real de agendamentos, independente do período).
--
-- Overloading: a versão text-only do 0011 precisa ser removida pra evitar
-- `42725: function is not unique` quando o front chamar com args nomeados
-- que casam com as duas assinaturas.

drop function if exists public.get_whatsapp_summary(text);
...
declare
  ...
  v_agend_delta numeric := 0; -- KPI é total, delta de período não se aplica
...
begin
  ...
  -- Agendamentos = total existente hoje (não filtra por período)
  ...
  -- Tempo médio de resposta entre 1ª msg do cliente NO PERÍODO e 1ª resposta posterior
  with allowed as (
  ...
  -- ============ Deltas: período vs janela imediatamente anterior ============
  ...
  -- ============ Série: 1 ponto por dia dentro do período ============
  with dias as (
  ...
  -- ============ Série: taxa de conversão semanal dentro do período ============
  with semanas as (
