-- 0021_whatsapp_response_time_new_leads.sql
-- Fix estrutural do tempo_resposta_min: restringe a threads CRIADAS
-- dentro do período (SLA de primeira resposta pra leads novos).
--
-- Motivo: no 0020 o CTE `allowed` incluía todas as threads (sem filtro
-- de período). Clientes recorrentes que só escreveram msgs mid-conversa
-- entravam com t_cliente = primeira msg deles na janela, e t_resp era
-- a próxima msg do atendente — o que subestima brutalmente o SLA de
-- atendimento porque a resposta anterior (fora da janela) não conta.
--
-- Nova definição:
--   allowed = threads criadas dentro da janela.
--   first_client = primeira msg do cliente EM QUALQUER momento (não
--                  precisa filtrar por hora, já que a thread nasceu
--                  dentro do período).
--   first_resp = primeira msg do atendente > first_client.
--
-- Isso alinha com "SLA de primeira resposta pra lead novo", que é a
-- métrica útil pro salão.
