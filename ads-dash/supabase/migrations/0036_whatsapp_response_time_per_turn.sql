-- 0036_whatsapp_response_time_per_turn.sql
-- Adiciona `tempo_resposta_turno_min` no get_whatsapp_summary — mediana
-- do tempo entre CADA msg do cliente e a próxima resposta do atendente
-- na mesma thread. Complementa `tempo_resposta_min` (que mede só o 1º
-- turno de threads criadas no período).
--
-- Motivo: SLA de primeira resposta pega quão rápido o atendente reage a
-- lead novo, mas não detecta "responde rápido no início e depois some".
-- Tempo médio por turno mede a consistência do atendimento ao longo de
-- toda a conversa.
...
begin
  ...
  -- SLA de primeira resposta: só threads criadas no período.
  -- Mediana em minutos + % com resposta em ≤10min.
  with allowed as (
  ...
  -- Tempo médio POR TURNO: pra cada msg do cliente dentro do período,
  -- tempo até a próxima msg do atendente na MESMA thread. Diferente do
  -- SLA de primeira resposta acima — aqui todos os turnos contam, o que
  -- captura o padrão "atendente some no meio da conversa".
