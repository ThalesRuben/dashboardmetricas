-- 0037_whatsapp_response_time_fixes.sql
-- Corrige dois problemas identificados após rodar 0036 em prod:
--
-- 1. tempo_resposta_turno_min inflado (~22 dias): o CROSS JOIN LATERAL
--    casava a próxima msg do atendente por mais distante que estivesse.
--    Fix: limita janela a 24h. Msgs sem resposta em 24h são descartadas
--    do cálculo (mediana representa "quanto tempo demora quando responde
--    mesmo").
--
-- 2. taxa_resposta / tempo_resposta_min desalinhados: v_threads_resp
--    contava atendente msgs pra QUALQUER thread com msg no período,
--    incluindo threads antigas. Já first_client/first_resp só olhavam
--    threads criadas no período. Isso dava taxa_resposta=3.5% e
--    tempo_resposta_min=0 juntos (as threads respondidas eram velhas).
--    Fix: v_threads_resp agora restringe a threads criadas no período,
--    alinhando com a semântica de "SLA de primeira resposta pra lead
--    novo" que foi estabelecida na 0021.
...
begin
  ...
  -- FIX #2: v_threads_resp agora restringe a threads CRIADAS no período,
  -- alinhando com first_client/first_resp abaixo. Semântica: "quantos
  -- leads novos no período receberam alguma resposta".
  select count(distinct m.thread_id) into v_threads_resp
    from whatsapp_msgs m
    join whatsapp_threads t on t.id = m.thread_id
    where m.autor = 'atendente'
      and t.criado_em >= v_from and t.criado_em <= v_to
      and (p_inbox_phone is null or t.inbox_phone = p_inbox_phone);
  ...
  -- FIX #1: turno restringido a janela de 24h. Msgs sem resposta nessa
  -- janela ficam de fora do cálculo (mediana descreve "quanto demora
  -- quando o atendente responde de fato").
  with allowed_turno as (
    select id from whatsapp_threads
    where (p_inbox_phone is null or inbox_phone = p_inbox_phone)
  ),
  pares as (
    select extract(epoch from (prox.hora - mc.hora)) / 60.0 as minutos
    from whatsapp_msgs mc
    join allowed_turno a on a.id = mc.thread_id
    cross join lateral (
      select ma.hora
      from whatsapp_msgs ma
      where ma.thread_id = mc.thread_id
        and ma.autor = 'atendente'
        and ma.hora > mc.hora
        and ma.hora <= mc.hora + interval '24 hours'
      order by ma.hora
      limit 1
    ) prox
    where mc.autor = 'cliente'
      and mc.hora >= v_from and mc.hora <= v_to
  )
  select coalesce(round(percentile_cont(0.5) within group (order by minutos))::int, 0)
    into v_tempo_resp_turno
  from pares;
  ...
  return jsonb_build_object(
    'resumo', jsonb_build_object(
      ...
      'tempo_resposta_turno_min', v_tempo_resp_turno,
